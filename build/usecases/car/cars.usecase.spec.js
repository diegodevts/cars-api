...
var UpdateCarUseCase = class {
  ...
  async handle(data, id2) {
    const updatedCar = await this.genericRepository.update(data, id2);
    if (!updatedCar) {
      throw new ResourceNotFoundError();
    }
    return updatedCar;
...
describe("Car Usecase", () => {
  beforeEach(async () => {
    ...
    await createUseCase.handle({
      color: "black",
      doors: 4,
      fuel_type: import_client.FUEL_TYPE.GASOLINE,
      model_id: (0, import_crypto2.randomUUID)(),
      year: "2020"
    });
  });
  it("should be able to create a car", async () => {
    globalExpect(carsRepository.items.length).toEqual(1);
  });
  ...
  it("should be able to update a car by id", async () => {
    ...
    globalExpect(car.id).toEqual(id);
    globalExpect(car.color).toBe("white");
  });
  it("should be able to delete a car by id", async () => {
    await deleteUseCase.handle(id);
    globalExpect(carsRepository.items.length).toEqual(0);
  });
