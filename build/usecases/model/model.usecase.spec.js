...
${job.error.stack.split("\n").slice(matchedLineIndex + 1).join("\n")}`;
  ...
  function firstTimerInRange(clock, from, to) {
    ...
    let id2, isInRange;
    for (id2 in timers2) {
      if (timers2.hasOwnProperty(id2)) {
        isInRange = inRange(from, to, timers2[id2]);
        if (isInRange && (!timer || compareTimers(timer, timers2[id2]) === 1)) {
          timer = timers2[id2];
        }
      }
    }
  ...
  function firstTimer(clock) {
    ...
    for (id2 in timers2) {
      if (timers2.hasOwnProperty(id2)) {
        if (!timer || compareTimers(timer, timers2[id2]) === 1) {
          timer = timers2[id2];
        }
      }
    }
  ...
  function clearTimer(clock, timerId, ttype) {
    ...
    const id2 = Number(timerId);
    if (Number.isNaN(id2) || id2 < idCounterStart) {
...
To automatically clean-up native timers, use \`shouldClearNativeTimers\`.`
    ...
    if (clock.timers.hasOwnProperty(id2)) {
      const timer = clock.timers[id2];
      if (timer.type === ttype || timer.type === "Timeout" && ttype === "Interval" || timer.type === "Interval" && ttype === "Timeout") {
        delete clock.timers[id2];
      } else {
  ...
  function createClock(start, loopLimit) {
    ...
    clock.setSystemTime = function setSystemTime(systemTime) {
      ...
      clock.now = newNow;
      nanos = 0;
      for (id2 in clock.timers) {
        if (clock.timers.hasOwnProperty(id2)) {
          timer = clock.timers[id2];
...
var UpdateModelUseCase = class {
  ...
  async handle(data, id2) {
    const updatedModel = await this.genericRepository.update(data, id2);
    if (!updatedModel) {
      throw new ResourceNotFoundError();
    }
    return updatedModel;
...
var DeleteModelUseCase = class {
  ...
  async handle(id2) {
    await this.genericRepository.delete(id2);
  }
};

// src/usecases/model/model.usecase.spec.ts
